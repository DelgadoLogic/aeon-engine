...
AeonSettings Defaults() {
    ...
    s.gpc_header_enabled    = true;
    s.doh_enabled           = true;
    strcpy_s(s.doh_provider, "https://1.1.1.1/dns-query");
    ...
    // Download dir: %USERPROFILE%\Downloads\Aeon
    char profile[MAX_PATH] = {};
    SHGetFolderPathA(nullptr, CSIDL_PROFILE, nullptr, 0, profile);
    _snprintf_s(s.download_dir, sizeof(s.download_dir), _TRUNCATE,
        "%s\\Downloads\\Aeon", profile);
    CreateDirectoryA(s.download_dir, nullptr);

    s.ask_per_download      = true;
    ...
    strcpy_s(s.search_engine,  "https://search.brave.com/search?q=");
    s.theme                 = 'A'; // auto
    s.hardware_accel        = true;
    s.auto_update           = true;
    strcpy_s(s.update_channel, "stable");

    // Read telemetry from HKLM (shared with LogicFlow)
...
AeonSettings Load() {
    ...
    s.gpc_header_enabled     = ReadJsonBool(buf, "gpc_header_enabled",    true);
    s.doh_enabled            = ReadJsonBool(buf, "doh_enabled",           true);
    ReadJsonStr(buf, "doh_provider", s.doh_provider, sizeof(s.doh_provider),
    ...
    ReadJsonStr(buf, "tor_socks_addr", s.tor_socks_addr, sizeof(s.tor_socks_addr),
        "127.0.0.1:9150");
    ReadJsonStr(buf, "download_dir", s.download_dir, sizeof(s.download_dir), "");
    s.max_parallel_downloads = ReadJsonInt(buf, "max_parallel_downloads", 4);
    ...
    ReadJsonStr(buf, "search_engine", s.search_engine, sizeof(s.search_engine),
        "https://search.brave.com/search?q=");
    char themeStr[4] = {"A"};
    ReadJsonStr(buf, "theme", themeStr, sizeof(themeStr), "A");
    s.theme = themeStr[0];
    s.auto_update = ReadJsonBool(buf, "auto_update", true);
    ReadJsonStr(buf, "update_channel", s.update_channel, sizeof(s.update_channel),
        "stable");
...
bool Save(const AeonSettings& s) {
    ...
    fprintf(f, "  \"gpc_header_enabled\": %s,\n",     s.gpc_header_enabled    ? "true" : "false");
    ...
    fprintf(f, "  \"max_parallel_downloads\": %d,\n", s.max_parallel_downloads);
